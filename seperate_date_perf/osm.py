...
def get_osrm_distance_matrix(warehouse_location, order_data_w, mode="driving"):
    ...
    url = f"http://localhost:5000/table/v1/{mode}/{coord_string}?annotations=distance"
    # Send the request to OSRM API
    response = requests.get(url)
    # Check if the response is successful
    if response.status_code != 200:
        raise Exception(
            f"OSRM request failed with status code {response.status_code}: {response.text}"
        )
...
def get_osrm_travel_time_matrix(warehouse_location, order_data_w, mode="driving"):
    ...
    url = f"http://localhost:5000/table/v1/{mode}/{coord_string}?annotations=duration"
    # Send the request to OSRM API
    response = requests.get(url)
    # Check if the response is successful
    if response.status_code != 200:
    ...
    for day, truck_routes in daily_truck_routes.items():
        for truck_route in truck_routes:
            if truck_route:  # Check if the truck has a non-empty route
                first_route_coords = truck_route[
                    0
                ]  # Get the first coordinate of the first non-empty route
                break
                ...
                if route_data:  # If route data is successfully retrieved
                    ...
                    if decoded_route:
                        ...
                        for i, coord in enumerate(truck_route_coords):
                            ...
                            folium.Marker(
                                coord, popup=popup_text, icon=folium.Icon(color=color)
                            ).add_to(feature_group)
...
def create_map_tree(warehouse_location, daily_routes, colors, output_path):
    ...
    for date in list(daily_routes.keys()):
        ovt["children"].append(
            {"label": date, "select_all_checkbox": True, "children": []}
        )
        a = 0
        for i in range(1, len(daily_routes[date])):
            ...
            if daily_routes[date][i]:
                ovt["children"][x]["children"].append(
                    {"label": f"Truck {i}", "select_all_checkbox": True, "children": []}
                )
